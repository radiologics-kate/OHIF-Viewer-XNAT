...
const allCornerstoneEvents = [
  ...
  "cornerstonetoolsmultitouchstart",
  "cornerstonetoolsmultitouchstartactive",
...
const loadDisplaySetIntoViewport = (data, templateData) => {
  ...
  if (!data.study || !data.displaySet || !data.element) {
    OHIF.log.warn(
      "loadDisplaySetIntoViewport: No Study, Display Set, or Element provided"
    );
    return;
  }
  ...
  stack = {
    currentImageIdIndex:
      data.currentImageIdIndex > 0 && data.currentImageIdIndex < imageIds.length
  ...
  const isUpdateMetadataDefined =
    metadataProvider && typeof metadataProvider.updateMetadata === "function";
  ...
  imagePromise.then(
    image => {
      ...
      try {
        ...
        OHIF.log.warn(
          "Viewport destroyed before loaded image could be displayed"
        );
        return;
      }
      ...
      const instanceClassViewport = getInstanceClassDefaultViewport(
        instance,
        enabledElement,
        image.imageId
      );
      ...
      if (
        multiframeMetadata &&
        multiframeMetadata.isMultiframeImage &&
        multiframeMetadata.averageFrameRate > 0
      ) {
      ...
      const onImageRendered = event => {
        ...
        if (!layoutManager.viewportData[viewportIndex]) {
          OHIF.log.warn(
            `onImageRendered: LayoutManager has no viewport data for this viewport index?: ${viewportIndex}`
          );
        }
      ...
      const onNewImage = event => {
        ...
        layoutManager.viewportData[viewportIndex].imageId =
          eventData.enabledElement.image.imageId;
        ...
        if (stack && stack.data.length && stack.data[0].imageIds.length > 1) {
          const imageIdIndex = stack.data[0].imageIds.indexOf(
            templateData.imageId
          );
          layoutManager.viewportData[
            viewportIndex
      ...
      const sendActivationTrigger = event => {
        ...
        const element =
          (eventData && eventData.element) || (event && event.currentTarget);
        if (!element) return;
        const $element = $(element);
      ...
      if (imagePlane && imagePlane.frameOfReferenceUID) {
        ...
        if (activeTool === "crosshairs") {
          ...
          cornerstoneTools.store.state.enabledElements.forEach(
            cornerstoneToolsElement => {
              ...
              cornerstoneTools.addToolState(
                ...
                "crosshairs",
                {
                  synchronizationContext:
                    crosshairsSynchronizers.synchronizers[
      ...
      if (
        ...
        typeof data.renderedCallback === "function"
      ) {
...
Template.imageViewerViewport.onRendered(function() {
  ...
  this.$element
    .siblings(".imageViewerLoadingIndicator")
    .css("display", "block");
  ...
  /* JamesAPetts - Disable this as it kills a lot of cornerstone stuff.
