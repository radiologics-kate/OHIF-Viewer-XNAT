...
const resetViewportWithElement = element => {
  ...
  if (enabledElement.fitToWindow === false) {
    ...
    const instanceClassDefaultViewport = getInstanceClassDefaultViewport(
      instance,
      enabledElement,
      imageId
    );
...
const hasMultipleFrames = () => {
  ...
  const nImages =
    stackData.imageIds && stackData.imageIds.length
      ? stackData.imageIds.length
      : 1;
...
const showSyncSettings = () => {
  ...
  dialog.on("keydown", e => {
    if ((e.which = 27)) {
      // If Esc is pressed cancel and close the dialog
      cancelHandler();
    }
  });
