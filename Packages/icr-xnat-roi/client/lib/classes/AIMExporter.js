...
export class AIMExporter {
  ...
  async exportToXNAT() {
    ...
    const putCreateResourceUrl = `${
      this._assessorUrl
    }/resources/AIM?content=EXTERNAL&format=XML&description=AIM+instance+file&name=AIM&${csrfTokenParameter}`;
    ...
    // Upload resource (AIM)
    const putUploadAimUrl = `${this._assessorUrl}/resources/AIM/files/${
      this._name
    }.xml?inbody=true&content=EXTERNAL&format=XML&${csrfTokenParameter}`;
    await this._PUT_uploadAim(putUploadAimUrl, this._aimString).catch(error => {
      putFailed = true;
      console.log(error);
    });
    ...
    const getConvertRTStuctUrl = `${Session.get("rootUrl")}/xapi/roi/projects/${
      this._projectID
    }/collections/${this._label}?type=RTSTRUCT&${csrfTokenParameter}`;

    console.log("GET URL for RTSTRUCT conversion:");
    console.log(getConvertRTStuctUrl);

    await this._GET(getConvertRTStuctUrl).catch(error => {
      putFailed = true;
      console.log(error);
    });
    ...
    if (putFailed) {
      throw Error(
        "POST failed for AIM to RTSTUCT conversion, check logs above."
      );
    }
