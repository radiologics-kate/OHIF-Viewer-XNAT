...
/**
 * displayROICollectionBuilderDialog - Opens a dialog which lets the user
 * select a set of one or more ROIs to export to XNAT as an roiCollection.
 *
 ...
 */
export default function displayROICollectionBuilderDialog(label) {
  ...
  const confirm = dialog.getElementsByClassName(
    "roi-collection-builder-export-button"
  )[0];
  const cancel = dialog.getElementsByClassName(
    "roi-collection-builder-cancel"
  )[0];
  const textInput = dialog.getElementsByClassName(
    "roi-collection-builder-text-input"
  )[0];
