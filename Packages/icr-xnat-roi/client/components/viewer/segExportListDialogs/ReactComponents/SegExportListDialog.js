...
import SegListItem from "./segListItem.js";

const brushModule = cornerstoneTools.store.modules.brush;

export default class SegExportListDialog extends React.Component {
  constructor(props = {}) {
    super(props);
  ...
  _closeDialog() {
    const dialog = document.getElementById("segExportListDialog");
    dialog.close();
  }
  ...
  render() {
    ...
    if (exporting) {
      ...
      segExportListBody = (
        <table className="peppermint-table">
          <tbody>
            {roiCollectionInfo ? (
              <tr className="seg-export-list-collection-info">
    ...
    return (
      <div>
        <div className="seg-export-list-header">
          <h3>Export Segmentations</h3>
          {!exporting && (
            <a
              className="seg-export-list-cancel btn btn-sm btn-secondary"
        ...
        <div className="seg-export-list-body">{segExportListBody}</div>

        <hr />

        {!exporting && (
          <div className="seg-export-list-footer">
            <label>Name</label>
            <input
